package hr.fer.zemris.test.smallTest;
...
import hr.fer.zemris.image.UniqueEnglishLetterMapper;
import hr.fer.zemris.io.ImageFinder;
import hr.fer.zemris.learn.*;
 ...
 * @author Filip Gulan
 */
public class EnglishUpercaseOneToOneHroziontal_SmallTest {

    private static final int INPUT_LAYER = 90;
    ...
    private static final String NNET_NAME = "Horizontal_Cross_5_Max_Index_Unique_English_Uppercase_Points_small_test.nnet";
    private static final String LEARNING_PATH = "/Users/filipgulan/FER/Zavrsni rad/Obrada2/";
    private static final String TEST_PATH = "/Users/filipgulan/FER/Zavrsni rad/Test2/";
    private static boolean learn = false;
    private static Map<Integer, Integer> errorMap = new TreeMap<>();
    private static EnglishCharacterMapper mapper = new EnglishCharacterMapper();
...
//        }
    ...
    private static DataSet loadDataSet(String path) {
        ...
        for (InputImage image : images) {
            int index = UniqueEnglishLetterMapper.mapLetter(image.getSymbol());
            if (index == - 1) continue;

            IFeatureVector vector = new HorizontalFetureVector(image);
    ...
    private static boolean isOutputSame(double[] netOutput, double[] desiredOutput) {
        ...
        if (!learn) {
            if (maxIndex != index) {
                System.out.println(mapper.mapInteger(index) + " as " + mapper.mapInteger(maxIndex));
            }
        }
